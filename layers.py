...
class Dense(nn.Module):
    def __init__(self, input_dim, output_dim,device, dropout=0.0, chnl=8):
        ...
        self.fc = nn.Linear(self.input_dim, self.output_dim).to(self.device)
...
class Bias(nn.Module):
    def __init__(self, dim,device):
        ...
        self.device = device
        self.bias = nn.Parameter(torch.zeros(dim)).to(self.device)
    def forward(self, x):
        x = x + self.bias
        return x
...
class CrossCompressUnit(nn.Module):
    def __init__(self, dim, device):
        ...
        self.device = device
        self.dim = dim
        self.fc_vv = nn.Linear(dim, 1, bias=False).to(self.device)
        self.fc_ev = nn.Linear(dim, 1, bias=False).to(self.device)
        self.fc_ve = nn.Linear(dim, 1, bias=False).to(self.device)
        self.fc_ee = nn.Linear(dim, 1, bias=False).to(self.device)

        self.bias_v = Bias(dim,self.device)
        self.bias_e = Bias(dim,self.device)
